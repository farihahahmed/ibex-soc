// ============================================================================
// tb_apb_deep.sv - deep diagnostic: watch the BRIDGE'S INTERNAL STATE during
// GPIO-then-UART writes. I probe u_bridge.state, addr_q, and the AHB handshake
// so I can see exactly why the UART write is lost. Bounded.
// ============================================================================
`timescale 1ns/1ps
module tb_apb_deep;
    ...
    // probe the bridge's internal state and the AHB interconnect's registered region
    always @(posedge clk) begin
        #1;
        $display("t=%0t req=%b gnt=%b HREADY=%b | HSEL=%b brHSEL=%b slvHADDR=0x%05h slvHWRITE=%b slvHTRANS=%b | br_state=%0d addr_q=0x%05h | PSEL=%b PEN=%b u_PSEL=%b g_PSEL=%b",
            $time, req, gnt, HREADY, HSEL, (HSEL[1]|HSEL[2]), slv_HADDR[19:0], slv_HWRITE, slv_HTRANS,
            u_bridge.state, u_bridge.addr_q[19:0], PSEL, PENABLE, uart_PSEL, gpio_PSEL);
    end
