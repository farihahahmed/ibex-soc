// trace the WRITE DATA path: adapter wdata_q -> HWDATA -> bridge -> PWDATA -> uart
`timescale 1ns/1ps
module tb_wdata;
    localparam int NUM_IO=8, CLK_FREQ=8, BAUD_RATE=1;
    logic clk, rst_n;
    logic req, gnt, we, rvalid; logic [3:0] be; logic [31:0] addr, wdata, rdata;
    ...
    always @(posedge clk) begin
        #1;
        $display("t=%0t req=%b gnt=%b we=%b | HWDATA=0x%08h adap_wq=0x%08h | u_PSEL=%b PEN=%b PWDATA=0x%08h | br_wq=0x%08h",
            $time, req, gnt, we, HWDATA, u_adapter.wdata_q, uart_PSEL, PENABLE, PWDATA, u_bridge.wdata_q);
    end
