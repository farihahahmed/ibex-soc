...
module rst_sync (
    input  logic clk,          // my system clock. Both flops run on this.
    input  logic rst_n_in,     // the raw reset coming in from outside (active-low, async).
    output logic rst_n_out     // the clean, synchronized reset going out to the rest of the chip.
);

    logic sync_ff1;            // first flip-flop. This one catches the async reset and may go metastable.
    logic sync_ff2;            // second flip-flop. Samples flop1 a cycle later, once it's settled. This is my clean output.
