`timescale 1ns/1ps
module tb_chip_gate_v2;
    localparam int NUM_OUT=5, NUM_IN=2, CLK_FREQ=8, BAUD_RATE=1;
    ...
    initial begin
        ...
        rst_n=0; repeat(8) @(negedge clk); wake_all; rst_n=1; repeat(6) @(negedge clk); wake_all;
        ...
        $display("--------------------------------------------------");
        if (errors==0) $display("PASS: GATE NETLIST matches RTL (Columbia-style, scan-configured)!");
