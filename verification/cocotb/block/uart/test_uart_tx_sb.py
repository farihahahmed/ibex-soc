...
class UartTxSbTest(uvm_test):
    ...
    async def run_phase(self):
        ...
        assert errors == 0, f"TX scoreboard errors={errors}"
        assert checked >= 1, "no frame scored"
        self.logger.info(f"*** TX SB PASS checked={checked} byte=0x{byte:02x} ***")
        self.drop_objection()


@cocotb.test()
async def test_uart_tx_sb(dut):
    ...
    dut_handle.DUT = dut
    await uvm_root().run_test("UartTxSbTest")
