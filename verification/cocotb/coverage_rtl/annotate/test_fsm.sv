//      // verilator_coverage annotation
        // test_fsm.sv - Columbia-style 3-mode clock-gating FSM.
        // Modes (from scan-written cfg_mode): 0=IDLE (clk suppressed), 1=RUN (clk passes),
        // 2=COUNTDOWN (clk passes for cfg_count cycles then gates).
        module test_fsm (
 117488     input  logic        clk,
 000039     input  logic        rst_n,
 000038     input  logic        cfg_load,
 001057     input  logic [1:0]  cfg_mode_in,
 001019     input  logic [15:0] cfg_count_in,
 103918     output logic        cpu_clk,
 000038     output logic        scan_owns_mem,
~000038     output logic [1:0]  mode_o
        );
            localparam logic [1:0] IDLE=2'd0, RUN=2'd1, COUNTDOWN=2'd2;
        
~000038     logic [1:0]  mode;
%000000     logic [15:0] count;
 000038     logic        run_gate;
        
 117526     always_ff @(posedge clk or negedge rst_n) begin
 117018         if (!rst_n) begin
 000508             mode  <= IDLE;
 000508             count <= 16'd0;
 000038         end else if (cfg_load) begin
 000038             mode  <= cfg_mode_in;
 000038             count <= cfg_count_in;
~116980         end else if (mode == COUNTDOWN && count != 16'd0) begin
%000000             count <= count - 16'd1;
                end
            end
        
 117527     always_comb begin
 117527         case (mode)
 103956             RUN:       run_gate = 1'b1;
%000000             COUNTDOWN: run_gate = (count != 16'd0);
 013571             default:   run_gate = 1'b0;
                endcase
            end
        
            // glitch-free clock gate: sample the enable on the FALLING edge of clk so it
            // is stable while clk is high (no chopped pulses). Neg-edge flop maps to a
            // standard cell -- no latch inference.
 000038     logic run_gate_q;
 117487     always_ff @(negedge clk or negedge rst_n) begin
 117018         if (!rst_n) run_gate_q <= 1'b0;
 117018         else        run_gate_q <= run_gate;
            end
        
            assign cpu_clk       = clk & run_gate_q;
            assign scan_owns_mem = (mode == IDLE);
            assign mode_o        = mode;
        endmodule
        
