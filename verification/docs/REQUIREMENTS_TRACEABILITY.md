...
| R-BOOT-13 | Clean recovery after reset asserted mid-operation | `test_reset_stress` — reset asserted partway through a scan-load and mid-RUN; asserts the control plane returns to IDLE with scan owning memory, and the chip is functional afterwards | CHIP | NEG | **Closed** |
...
| R-METH-03 | One-button official gate | `./run_all_verify.sh` — 52 suites, EXIT=0 | Closed |
...
| Command | Covers |
|---------|--------|
| `./run_all_verify.sh` | The whole gate: 52 suites. Chip smoke/random/firmware/dmem/scan/clkgen/pcpi, negative/corner/stress, and all 11 block suites (uart, gpio, spi, mem, fsm, scan, scan_fsm, ahb, ahb_to_apb, apb_decoder, clkgen) |
...
- **R-GL-02** — `make gl-firmware` scan-loads real firmware (`fw_gpio_walk`)
  ...
  correctness by the RTL cocotb gate (52 suites). A full GL-execution close
